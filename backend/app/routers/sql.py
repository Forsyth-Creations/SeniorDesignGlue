...
async def sql(query: Query, db: Session = Depends(get_db)):
    try:
        ...
        if query.remember:
            ...
            db.execute(text("CREATE TABLE IF NOT EXISTS common_queries (query TEXT, execution_time TEXT)"))
            
            # Check to make sure this query is not already in the database
            query_exists = db.execute(text("SELECT query FROM common_queries WHERE query = :query"), {"query": query.query}).fetchone()
            if not query_exists:
                execution_time = str(time.time())
                db.execute(text("INSERT INTO common_queries (query, execution_time) VALUES (:query, :execution_time)"), {"query": query.query, "execution_time": execution_time})
                db.commit()
...
async def delete_query(query: str, db: Session = Depends(get_db)):
    try:
        db.execute(text("DELETE FROM common_queries WHERE query = :query"), {"query": query})
        db.commit()
